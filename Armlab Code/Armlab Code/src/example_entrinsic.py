...
# SVD modified calculate A
A_svd_mod = recover_homogeneous_transform_svd_modified(points_world, points_camera)
points_transformed_svd_modified = np.dot(
    np.linalg.inv(A_svd_mod), np.transpose(np.column_stack((points_camera, points_ones))))
...
print("\nRigid Body (SVD Modified): \n")
print(A_svd_mod)
print(points_transformed_svd_modified.astype(int))
